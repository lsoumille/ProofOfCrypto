...
public class Program {
	...
	/**
	 * Remove the first instruction of the list == toDoAfter and create a deep copy
	 *
	 * @return
	 */
	public Program getNextInstructions() {
		Program toDoAfter = new Program();
		List<GrammarParser.CContext> newInstructions = new ArrayList<>();
		for (int i = 1; i < this.instructions.size(); ++i) {
			newInstructions.add(this.instructions.get(i));
		}
	...
	public String toString() {
		String ret = "";
		for (GrammarParser.CContext c : this.instructions) {
			ret += c.getText();
		}
	...
	@Override
	public int hashCode() {
		return this.instructions != null || this.instructions.isEmpty() ? this.toString().hashCode() : 0;
	}
}
