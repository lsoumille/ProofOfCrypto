...
public class Distribution {
	...
	public Distribution(Distribution d) {
		this.configWithProbability = new HashMap<>(d.getConfigWithProbability());
	}

	public Map<Configuration, Double> getConfigWithProbability() {
		return this.configWithProbability;
	}

	public List<Configuration> getConfigurations() {
		return new ArrayList<>(this.configWithProbability.keySet());
	}

	public Double getProba(Configuration c) {
		return this.configWithProbability.get(c);
	}
	...
	public void addConfiguration(Configuration c, Double probability) {
		...
		this.configWithProbability.put(c, probability);
	}
	...
	public boolean equals(Object o) {
		...
		for (Map.Entry<Configuration, Double> entry : this.configWithProbability.entrySet()) {
			if (that.getConfigWithProbability().get(entry.getKey()) == null
					|| !entry.getValue().equals(that.getConfigWithProbability().get(entry.getKey()))) {
				return false;
			}
		}
		for (Map.Entry<Configuration, Double> entry : that.getConfigWithProbability().entrySet()) {
			if (this.configWithProbability.get(entry.getKey()) == null
					|| !entry.getValue().equals(this.configWithProbability.get(entry.getKey()))) {
				return false;
			}
		}
		return true;
	...
	/**
	 * Return true if the parameter is a distribution and is equivalent for the vars
	 *
	 * @param o
	 * @param vars
	 * @return
		...
		for (String var : vars) {
			...
			for (Map.Entry<Configuration, Double> eThis : this.configWithProbability.entrySet()) {
				if (eThis.getKey().getMemory().getValForVar(var) != null) {
					Map.Entry<String, Integer> entry = new AbstractMap.SimpleEntry<>(var, eThis.getKey().getMemory().getValForVar(var));
			...
			for (Map.Entry<Configuration, Double> eThat : that.configWithProbability.entrySet()) {
				if (eThat.getKey().getMemory().getValForVar(var) != null) {
					Map.Entry<String, Integer> entry = new AbstractMap.SimpleEntry<>(var, eThat.getKey().getMemory().getValForVar(var));
